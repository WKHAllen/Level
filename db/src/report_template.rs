...
mod tests {
    ...
    async fn test_report_template() {
        ...
        // Create
        let mut template1 = ReportTemplate::create(&db, "Template 1", "First template").await;
        let mut template2 = ReportTemplate::create(&db, "Template 2", "Second template").await;

        // Get
        let template3 = ReportTemplate::get(&db, &template1.id).await.unwrap();
        assert_eq!(template3, template1);
        let template4 = ReportTemplate::get(&db, &template2.id).await.unwrap();
        assert_eq!(template4, template2);
        assert!(ReportTemplate::get(&db, "").await.is_none());

        // List
        let templates = ReportTemplate::list(&db).await;
        ...
        // Set name
        template1.set_name(&db, "Not template 1").await;
        assert_eq!(&template1.name, "Not template 1");
        let template7 = ReportTemplate::get(&db, &template1.id).await.unwrap();
        assert_eq!(template7, template1);

        // Set description
        template1
            .set_description(&db, "Not template 1 description")
            .await;
        assert_eq!(
            template1.description.as_ref().unwrap().as_str(),
            "Not template 1 description"
        );
        let template8 = ReportTemplate::get(&db, &template1.id).await.unwrap();
        ...
        assert!(ReportTemplate::get(&db, &template_id1).await.is_some());
        template1.delete(&db).await;
        assert!(ReportTemplate::get(&db, &template_id1).await.is_none());
        let template_id2 = template2.id.clone();
        assert!(ReportTemplate::get(&db, &template_id2).await.is_some());
        template2.delete(&db).await;
        assert!(ReportTemplate::get(&db, &template_id2).await.is_none());
