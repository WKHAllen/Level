...
mod tests {
    ...
    async fn test_category() {
        ...
        // Create
        let mut category1 = Category::create(&mut db, "Category #1", "The first category").await;
        let category2 = Category::create(&mut db, "Category #2", "The second category").await;
        let category3 = Category::create(&mut db, "Category #3", "The third category").await;

        // Get
        let category4 = Category::get(&mut db, &category1.id).await.unwrap();
        assert_eq!(category4, category1);
        let category5 = Category::get(&mut db, &category2.id).await.unwrap();
        assert_eq!(category5, category2);
        let category6 = Category::get(&mut db, &category3.id).await.unwrap();
        assert_eq!(category6, category3);
        assert!(Category::get(&mut db, "").await.is_none());
        ...
        // List
        let categories = Category::list(&mut db).await;
        ...
        // Set name
        category1.set_name(&mut db, "name 1").await;
        assert_eq!(&category1.name, "name 1");
        let category13 = Category::get(&mut db, &category1.id).await.unwrap();
        assert_eq!(category13, category1);

        // Set description
        category1.set_description(&mut db, "description 1").await;
        assert_eq!(category1.description.as_ref().unwrap(), "description 1");
        let category14 = Category::get(&mut db, &category1.id).await.unwrap();
        assert_eq!(category14, category1);

        // Delete
        let category_id1 = category1.id.clone();
        assert!(Category::get(&mut db, &category_id1).await.is_some());
        category1.delete(&mut db).await;
        assert!(Category::get(&mut db, &category_id1).await.is_none());
        let category_id2 = category2.id.clone();
        assert!(Category::get(&mut db, &category_id2).await.is_some());
        category2.delete(&mut db).await;
        assert!(Category::get(&mut db, &category_id2).await.is_none());
        let category_id3 = category3.id.clone();
        assert!(Category::get(&mut db, &category_id3).await.is_some());
        category3.delete(&mut db).await;
        assert!(Category::get(&mut db, &category_id3).await.is_none());
